...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'bank_accounts') THEN
        RAISE EXCEPTION 'bank_accounts table does not exist';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'balances') THEN
        RAISE EXCEPTION 'balances table does not exist';
    END IF;
END $$;
